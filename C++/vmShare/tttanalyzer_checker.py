...
def allboards_test(process):
    res = "testing get_all_boards()\n"
    try:
        process.stdin.write("a\n")
        process.stdin.flush()
        #print(help(process.stdout.readlines))
        ans = process.stdout.readlines(3**9 * 10-1)
    except BrokenPipeError:
        res += "Your program crashed."
        return res,0
    ans.sort()
    all_boards_answer="".join(ans)
    h = hashlib.sha256()
...
def tttanalyzer_tester(program_to_run):
    ...
    res,basescore = tttstring_test(process)
    ...
    resa,basescore = allboards_test(process)
    res += resa
...
def test_original(source_file,executable_name):
    ...
    if C.returncode:
        print(str(C)+C.stderr.decode())

    T = run([executable_name],stderr=PIPE,stdout=PIPE)
...
if __name__ == '__main__':
    ...
    msg = test_original(PD['source_file'],"tttanalyzer_original")
    with open(PD['source_file']) as f:
        the_program = f.read()

    result = special_compile(the_program,'tttanalyzer_main_replaced.cpp',PD['program_to_run'])
